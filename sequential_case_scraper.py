...
def generate_appeal_case_numbers(year, start_seq=1, count=1000):
    yr = str(year)[-2:].zfill(2)
    return [f"ACIAS{yr}{str(i).zfill(5)}" for i in range(start_seq, start_seq + count)]
...
def scan_appeal(args, conn, year, scanned):
    yr = str(year)[-2:].zfill(2)
    prefix_label = f"ACIAS{yr}"
    case_nums = generate_appeal_case_numbers(year, args.start_seq, args.count)
    total = len(case_nums)
    found = 0
...
Examples:
  ...
  # Scan appeal cases ACIAS24xxxxx, ACIAS25xxxxx, etc.
  python sequential_case_scraper.py --appeal --year 2020 --end-year 2026 --count 5000
        """,
    )
    ...
    parser.add_argument("--appeal", action="store_true", help="Scan appeal cases (ACIAS prefix, 5-digit sequence)")
    ...
    if args.appeal:
        ...
        total = len(years)
        estimate = total * args.count
        est_time = estimate * args.delay
        print(f"Plan: {total} appeal year-prefixes, ~{estimate} requests, ~{est_time:.0f}s ({est_time/60:.1f} min)")
        ...
        for year in years:
            label = f"ACIAS{str(year)[-2:]}"
            t0 = time.time()
            found = scan_appeal(args, conn, year, scanned)
